		...
		if (ctx->multichannel) {
			if (!ctx->max_channels_specified) {
				ctx->max_channels = 2;
					...
					cifs_errorf(fc, "max_channels must be greater than 1 when multichannel is enabled\n");
					return -EINVAL;
			}
		} else {
			if (!ctx->max_channels_specified) {
				...
				cifs_errorf(fc, "max_channels must be equal to 1 when multichannel is disabled\n");
				return -EINVAL;
			}
		}
	} else {
...
static int smb3_reconfigure(struct fs_context *fc)
{
	...
	*/
	if ((ctx->multichannel != cifs_sb->ctx->multichannel) || (ctx->max_channels != cifs_sb->ctx->max_channels)){
		...
		rc = smb3_sync_ses_channels(ses,ses->server,false,true);
