...
static int smb3_reconfigure(struct fs_context *fc)
{
	...
	/*
	 * unix/nounix is negotiated with the server at connect, not
	 * reconfigurable at remount.  cifs_show_options() derives nounix from
	 * the runtime state, so a bare 'mount -o remount' replays it; carry the
	 * mounted value forward instead of letting the replay overwrite it.
	 */
	ctx->linux_ext = cifs_sb->ctx->linux_ext;
	ctx->no_linux_ext = cifs_sb->ctx->no_linux_ext;
