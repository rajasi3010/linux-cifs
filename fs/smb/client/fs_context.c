				       ...
				       struct smb3_fs_context *old_ctx, bool need_recon)
{
	...
	/* init default: noblocksnd = 0 */
	if (new_ctx->noblocksnd &&
	    new_ctx->noblocksnd != old_ctx->noblocksnd) {
		cifs_errorf(fc, "can not change noblocksend during remount\n");
		return -EINVAL;
	}
	/* init default: noautotune = 0 */
	if (new_ctx->noautotune &&
	    new_ctx->noautotune != old_ctx->noautotune) {
		cifs_errorf(fc, "can not change noautotune during remount\n");
		return -EINVAL;
	}
	/* init default: no_sparse = 0 */
	if (new_ctx->no_sparse &&
	    new_ctx->no_sparse != old_ctx->no_sparse) {
		cifs_errorf(fc, "can not change nosparse during remount\n");
		return -EINVAL;
	}
	/* init default: nodelete = 0 */
	if (new_ctx->nodelete &&
	    new_ctx->nodelete != old_ctx->nodelete) {
		cifs_errorf(fc, "can not change nodelete during remount\n");
		return -EINVAL;
	}
	/* has tracking flag cruid_specified */
	...
	/* init default: min_offload = 0 */
	if (new_ctx->min_offload &&
	    new_ctx->min_offload != old_ctx->min_offload) {
		cifs_errorf(fc, "can not change min_enc_offload during remount\n");
		return -EINVAL;
	}
	/* init default: snapshot_time = 0 */
	if (new_ctx->snapshot_time &&
	    new_ctx->snapshot_time != old_ctx->snapshot_time) {
		cifs_errorf(fc, "can not change snapshot during remount\n");
		return -EINVAL;
	}
	/* init default: max_credits = 0 */
	if (new_ctx->max_credits &&
	    new_ctx->max_credits != old_ctx->max_credits) {
		cifs_errorf(fc, "can not change max_credits during remount\n");
		return -EINVAL;
	}
	/* init default: handle_timeout = 0 */
	if (new_ctx->handle_timeout &&
	    new_ctx->handle_timeout != old_ctx->handle_timeout) {
		cifs_errorf(fc, "can not change handletimeout during remount\n");
		return -EINVAL;
	}
	/* has tracking flag got_ip */
	if (new_ctx->got_ip &&
	    memcmp(&new_ctx->dstaddr, &old_ctx->dstaddr, sizeof(new_ctx->dstaddr))) {
		cifs_errorf(fc, "can not change ip during remount\n");
		return -EINVAL;
	}
	/* init default: srcaddr is all zeros; check sa_family to detect if set */
	...
	/* init default: linux_ext = 0, no_linux_ext = 0 */
	if ((new_ctx->linux_ext || new_ctx->no_linux_ext) &&
	    (new_ctx->linux_ext != old_ctx->linux_ext ||
	     new_ctx->no_linux_ext != old_ctx->no_linux_ext)) {
		cifs_errorf(fc, "can not change unix during remount\n");
		return -EINVAL;
	}
	/* init default: nocase = 0 */
	if (new_ctx->nocase &&
	    new_ctx->nocase != old_ctx->nocase) {
	...
	/* init default: local_lease = 0 */
	if (new_ctx->local_lease &&
	    new_ctx->local_lease != old_ctx->local_lease) {
		cifs_errorf(fc, "can not change locallease during remount\n");
		return -EINVAL;
	}
	/* init default: sign = false */
	if (new_ctx->sign &&
	    new_ctx->sign != old_ctx->sign) {
		cifs_errorf(fc, "can not change sign during remount\n");
		return -EINVAL;
	}
	/* init default: ignore_signature = false */
	if (new_ctx->ignore_signature &&
	    new_ctx->ignore_signature != old_ctx->ignore_signature) {
		cifs_errorf(fc, "can not change ignore_signature during remount\n");
		return -EINVAL;
	}
	/* init default: seal = 0 */
	if (new_ctx->seal &&
	    new_ctx->seal != old_ctx->seal) {
		cifs_errorf(fc, "can not change seal during remount\n");
		return -EINVAL;
	}
	/* init default: nosharesock = false */
	if (new_ctx->nosharesock &&
	    new_ctx->nosharesock != old_ctx->nosharesock) {
		cifs_errorf(fc, "can not change nosharesock during remount\n");
		return -EINVAL;
	}
	/* init default: persistent = false, nopersistent = false */
	if ((new_ctx->persistent || new_ctx->nopersistent) &&
	    (new_ctx->persistent != old_ctx->persistent ||
	     new_ctx->nopersistent != old_ctx->nopersistent)) {
		cifs_errorf(fc, "can not change persistenthandles during remount\n");
		return -EINVAL;
	}
	/* init default: resilient = false */
	if (new_ctx->resilient &&
	    new_ctx->resilient != old_ctx->resilient) {
		cifs_errorf(fc, "can not change resilienthandles during remount\n");
		return -EINVAL;
	}
	/* init default: sockopt_tcp_nodelay = false */
	if (new_ctx->sockopt_tcp_nodelay &&
...
/*
 * Carry forward non-reconfigurable mount options from the original
 * context into the new remount context before smb3_fs_context_dup()
 * overwrites cifs_sb->ctx.  Without this, a bare remount would
 * silently reset these fields to their init defaults, causing
 * a subsequent remount that re-specifies the same option to be
					    ...
					    struct smb3_fs_context *old_ctx)
{
	...
	new_ctx->no_sparse = old_ctx->no_sparse;
	new_ctx->nodelete = old_ctx->nodelete;
	new_ctx->cred_uid = old_ctx->cred_uid;
	new_ctx->cruid_specified = old_ctx->cruid_specified;
	new_ctx->port = old_ctx->port;
	new_ctx->min_offload = old_ctx->min_offload;
	new_ctx->snapshot_time = old_ctx->snapshot_time;
	new_ctx->max_credits = old_ctx->max_credits;
	new_ctx->handle_timeout = old_ctx->handle_timeout;
	new_ctx->dstaddr = old_ctx->dstaddr;
	new_ctx->got_ip = old_ctx->got_ip;
	new_ctx->srcaddr = old_ctx->srcaddr;
	memcpy(new_ctx->target_rfc1001_name, old_ctx->target_rfc1001_name,
	       RFC1001_NAME_LEN_WITH_NULL);
	new_ctx->ops = old_ctx->ops;
	new_ctx->vals = old_ctx->vals;
	new_ctx->got_version = old_ctx->got_version;
	new_ctx->witness = old_ctx->witness;
	new_ctx->rootfs = old_ctx->rootfs;
	new_ctx->linux_ext = old_ctx->linux_ext;
	new_ctx->no_linux_ext = old_ctx->no_linux_ext;
