...
/**
* smb3_sync_ses_channels - Synchronize session channels with new configuration
...
*/
int smb3_sync_ses_channels(struct cifs_ses *ses, struct TCP_Server_Info *server, 
			bool from_reconnect, bool from_reconfigure)
{
	int rc = 0;
	/*
	...
	*/
	if (ses->chan_count < ses->chan_max) {
		...
		rc = cifs_chan_skip_or_disable(ses, server, from_reconnect, from_reconfigure);
	}
...
again:
	...
	if (ses->chan_count > 1 &&
		...
		rc = smb3_sync_ses_channels(ses, server,
...
skip_sess_setup:
	...
	if (!rc &&
		...
		if (ses->chan_max > ses->chan_count &&
			...
			smb3_sync_ses_channels(ses, server, from_reconnect, false);
		}
	} else {
		mutex_unlock(&ses->session_mutex);
	}
