...
unlink_target:
	...
	if (d_really_is_positive(target_dentry) && (rc == -EACCES || rc == -EEXIST)) {
		...
		if (tmprc) {
			/*
			 * Some servers will return STATUS_ACCESS_DENIED or
			 * STATUS_DIRECTORY_NOT_EMPTY when failing to rename
			 * a non-empty directory.  Make sure to propagate the
			 * appropriate error back to userspace.
			 */
